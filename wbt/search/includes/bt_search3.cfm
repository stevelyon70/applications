...
<cfset shardlist = ""/>
<cfset session.qresults = QueryNew( "bidID, pdf_exists, onviarefnum", "INTEGER, VARCHAR, INTEGER" ) />
	 	  ...
	 	  <cfcase value="1">
		   	<cfif not listcontains(project_stage, 12)>
				...
				<cfhttp result="result1" method="GET" charset="utf-8" url='#shard#/tpc'>
					...
					<cfhttpparam name="fl" type="formfield" value='bidID,pdf_exists:exists(pdf),onviarefnum'>
				</cfhttp>
		   ...
		   <cfcase value="4">
		   	<cfif not listcontains(project_stage, 12)>
				...
				<cfloop from="1" to="#arraylen(reslts_4)#" index="_i">
					...
					<cfset QuerySetCell(session.qresults, "onviarefnum", reslts_4[_i].onviarefnum[1]) />
				</cfloop>	
