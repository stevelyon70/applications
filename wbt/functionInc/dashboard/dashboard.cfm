...
<!--- cachedwithin="#CreateTimeSpan(0,0,30,0)#" --->
			...
			from pbt_project_master_gateway
				...
				inner join pbt_project_master_cats ppmc3 on ppmc3.bidID = pbt_project_master_gateway.bidID 
...
<!--- cachedwithin="#CreateTimeSpan(0,0,30,0)#" --->
			...
			and ppc.tagID in (#valuelist(session.model.formOtherStrucDropOptions.tagID)#)<!---(#session.user_tags#)--->
			<!--- site default---> 
			and ppmc3.tagID in (#session.default.tagId#)	
			</cfquery>
      </cfif>
