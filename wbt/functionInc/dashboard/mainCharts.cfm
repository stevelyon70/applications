...
<cfquery datasource="#application.datasource#" name="IndCount">
   ...
   WHERE 1=1  
    ...
    and ppmc.tagID in (select pbt_tags.tagID from pbt_tags inner join pbt_tag_packages pt on pt.tagid = pbt_tags.tagid inner join site_tag_xref x on pbt_tags.tagID = x.tagID  where pt.packageID= 1 and tag_parentID <> 0 and siteID = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#session.auth.siteID#" />) 
...
<cfquery datasource="#application.datasource#" name="commCount">
   ...
   WHERE 1=1  
    and (a.paintpublishdate >= '10/1/2016' and a.paintpublishdate <= '12/31/2016') 
    and (1 <> 1 or (a.stateid in (3,2,5,4,7,8,9,10,67,11,12,13,15,16,17,14,18,19,20,24,23,21,25,26,28,27,29,33,39,35,36,37,40,31,32,41,42,44,45,49,50,51,53,54,55,57,56,58,60,59,61)) ) 
    and ppmc.tagID in (select pbt_tags.tagID from pbt_tags inner join pbt_tag_packages pt on pt.tagid = pbt_tags.tagid inner join site_tag_xref x on pbt_tags.tagID = x.tagID where pt.packageID= 2 and tag_parentID <> 0 and siteID = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#session.auth.siteID#" />) 
    and a.verifiedpaint = 1
</cfquery>     
