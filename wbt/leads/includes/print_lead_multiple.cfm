	 ...
	 <cfif isdefined("session.auth.userID") and session.auth.userID NEQ ""><!---run if not coming from showcase--->
		...
		<cfelse>
			<CFQUERY NAME="GET_Sub_TAGS" DATASOURCE="#application.datasource#">
				...
				where 0=0 and siteID = <cfqueryparam cfsqltype="CF_SQL_INTEGER" value="#session.auth.siteID#" />
			</cfquery>
		<cfset variable.user_tags = valuelist(GET_Sub_TAGS.tagID)>
		</cfif>
						...
						<div class="row">	
							<div class="col-sm-12">	
								...
								<cfloop query="get_tags">			
									<cfset tagDisplay = application.tagsObj.tagDisplay(get_tags.tagID) />
									<cfif tagDisplay EQ ''>				
										<a href="#request.rootpath#search/?search_history=1&action=sresults&state=66&projecttype=1&project_stage=1&project_stage=2&project_stage=3&project_stage=4&tags=#get_tags.tagID#">
